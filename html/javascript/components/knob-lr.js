...
export class KnobLR extends HTMLElement {
  static get observedAttributes () {
    return ['tag', 'label', 'min', 'max', 'step', 'default', 'value', 'm']
  }
  ...
  constructor () {
    ...
    const template = document.querySelector('#template-knob-s')
  ...
  redraw () {
    ...
    const X0 = 0
    const Y0 = -r
    const X1 = -r * sin(angle)
    const Y1 = r * cos(angle)

    if (value < 0) {
      ring.setAttribute('d', `M${X0},${Y0} A${r},${r},0,0,0,${X1},${Y1}`)
    } else {
...
customElements.define('snyth-knob-lr', KnobLR)
