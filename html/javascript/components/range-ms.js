...
export class RangeMS extends HTMLElement {
  ...
  set value (v) {
    ...
    if (Number.isNaN(val)) {
      this.internal.value = Number.NaN
    } else {
      this.internal.value = Math.round(1000 * val) / 1000
    }

    input.value = fmt`${this.internal.value}`
  }
...
customElements.define('snyth-range-ms', RangeMS)
