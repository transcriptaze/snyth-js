...
export class Gain extends HTMLElement {
  static get observedAttributes () {
    return ['level-width', 'level-scale', 'min', 'max', 'step', 'default', 'value']
  }
  ...
  get clipping () {
    return this.internal.clipping
  }

  set clipping (v) {
    this.internal.clipping = v

    const shadow = this.shadowRoot
    const svg = shadow.querySelector('svg')
    const led = svg.querySelector('#led')
    const brightness = clamp(1.0 * this.internal.clipping, 0, 1.0)
  ...
  redraw (scope) {
    ...
    const θ0 = 2 * PI / 3
    const θ1 = 7 * PI / 3
    const dθ = this.level * (θ1 - θ0) * this.internal.levelScale
    const gradient = ctx.createConicGradient(120 * PI / 180, x, y)

    gradient.addColorStop(0.0 * 300 / 360, 'green')
    gradient.addColorStop(0.5 * 300 / 360, 'yellow')
    gradient.addColorStop(0.75 * 300 / 360, 'orange')
    gradient.addColorStop(1.0 * 300 / 360, 'red')
    gradient.addColorStop(301 / 360, '#222222ff')
    gradient.addColorStop(1, '#222222ff')
...
customElements.define('snyth-gain', Gain)
